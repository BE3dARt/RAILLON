class locomotive {
	...
	constructor(coordinates, layout, segment, subsegment, movingDirection, speed, heading, mesh, scene) {
		...
		const resultPromise = BABYLON.SceneLoader.ImportMeshAsync("", "https://raw.githubusercontent.com/BE3dARt/RAILBLAZER/main/assets/glb/", mesh + ".glb", scene);
		...
		resultPromise.then((load3D) => {
			...
			for (let i = 3; i < load3D.meshes.length; i++) {
				...
				// getBogiePositionNextMember(coordinatesReferenceBogie, layout, segment, subsegment, distance, scene)
				
				// Initialize every other bogie
				this.bogies.push(new bogie(result[0], layout, result[1], result[2], movingDirection, load3D.meshes[i]));
				// this.bogies.push(new bogie(result[0], layout, result[1], result[2], movingDirection, BABYLON.MeshBuilder.CreateBox("box", {}, scene)));
			}
